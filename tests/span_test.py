...
class TestSpan(unittest.TestCase):
    def test_duration(self):
        s = Span(
            datetime.datetime(2021, 4, 11, 11, 6, 5),
            datetime.datetime(2021, 4, 11, 11, 7, 21),
        )
    ...
    def test_get_spans_working(self):
        ...
        expected = [Span(events[0].timestamp, now)]
    ...
    def test_get_spans_idle_working(self):
        ...
        now = datetime.datetime(2021, 4, 11, 20, 30)
        spans = list(get_work_spans(events, now))
        expected = [Span(events[1].timestamp, now)]
        self.assertEqual(spans, expected)
    ...
    def test_get_spans_working_idle(self):
        ...
        spans = list(get_work_spans(events))
        expected = [Span(events[0].timestamp, events[1].timestamp)]
        self.assertEqual(spans, expected)
    ...
    def test_get_spans_working_working_idle(self):
        ...
        spans = list(get_work_spans(events))
        expected = [Span(events[0].timestamp, events[2].timestamp)]
        self.assertEqual(spans, expected)
    ...
    def test_get_spans_switching_projects(self):
        events = [
            Event(
                datetime.datetime(2021, 4, 11, 17, 30),
                "",
                Activity.WORKING,
                "Project A",
            ),
            Event(
                datetime.datetime(2021, 4, 11, 17, 40),
                "",
                Activity.WORKING,
                "Project B",
            ),
            Event(
                datetime.datetime(2021, 4, 11, 17, 45),
                "",
                Activity.WORKING,
                "Project A",
            ),
